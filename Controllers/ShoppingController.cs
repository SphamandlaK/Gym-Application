    ...
    public class ShoppingController : Controller
    {
        ...
        public ActionResult Payment_Successfull(int? id)
        {
            ...
            try
            {
                ...
                var sd = db.Customers.ToList().Find(x => x.Email == HttpContext.User.Identity.Name);
                try
                {
                    string myadd = db.Order_Addresses.Where(X => X.Order_ID == id).Select(Y => Y.street).SingleOrDefault();
                    if (myadd.Contains("123 west street, Durban, 4001"))
                    {
                        Sms.Send_SMS(sd.phone, "Hi " + sd.FirstName + " " + sd.LastName + "Your order was received and it will be ready for collection at our store, by" + " " + DateTime.Today.AddDays(1));
                    }
                    else
                    {
                        Sms.Send_SMS(sd.phone, "Hi " + sd.FirstName + " " + sd.LastName + "Your order was received and it will be delivered at" + " " + myadd + " " + "3 days time");
                    }
			...
			return RedirectToAction("GenerateQr");
        }

		public ActionResult GenerateQr()
		{
			var order = db.Orders.Where(x => x.Email == User.Identity.Name).ToList().OrderByDescending(x => x.Order_ID).FirstOrDefault();
			int ordid = order.Order_ID;
			string email = order.Email;
			string orderqr =email + ordid.ToString();


			QRCodeGenerator ObjQr = new QRCodeGenerator();

			QRCodeData qrCodeData = ObjQr.CreateQrCode(orderqr, QRCodeGenerator.ECCLevel.Q);
...

			{
				...
				bitMap.Save(Server.MapPath("~/Uploads/" + orderqr + "qrcode.png"), System.Drawing.Imaging.ImageFormat.Png);
			}

			OrderQr qrs = new OrderQr();
			qrs.qr = orderqr+"qrcode.png";
			db.OrderQrs.Add(qrs);
			db.SaveChanges();
			int id = qrs.Id;
			...
			return RedirectToAction("qrs");
		}

		public ActionResult qrs()
		{
			TempData.Keep("id");
			string idd = TempData["id"].ToString();
			TempData.Keep("id");
			int id = int.Parse(idd);
			return View(db.OrderQrs.Where(x=> x.Id == id).ToList());
		}
        ...
        public string PaymentLink(string totalCost, string paymentSubjetc, int order_id)
        {
            ...
            string paymentMode = ConfigurationManager.AppSettings["PaymentMode"], site, merchantId, merchantKey, returnUrl;
            //var cart = db.Carts.ToList().Find(x => x.cart_id == User.Identity.Name);

            //List<Cart_Item> items = db.Cart_Items.ToList().FindAll(x => x.cart_id == cart.cart_id);

            // totalCost = items.Sum(x => x.price * x.quantity).ToString();

            site = "https://sandbox.payfast.co.za/eng/process?";
            //merchant_id = "10014837";
            //merchant_key = "h88nigf6cqihv";
            merchantId = "10002201";
            merchantKey = "25lbpwmazv8rn";
            returnUrl = "https://localhost:44330/Shopping/Payment_Successfull/";
            ...
            stringBuilder.Append("&merchant_id=" + HttpUtility.HtmlEncode(merchantId));
            stringBuilder.Append("&merchant_key=" + HttpUtility.HtmlEncode(merchantKey));
            stringBuilder.Append("&return_url=" + HttpUtility.HtmlEncode("https://2020grp31.azurewebsites.net/Shopping/Payment_Successfull/" + order_id));
            stringBuilder.Append("&cancel_url=" + HttpUtility.HtmlEncode("https://2020grp31.azurewebsites.net/Shopping/Payment_Cancelled/" + order_id));
            stringBuilder.Append("&notify_url=" + HttpUtility.HtmlEncode(ConfigurationManager.AppSettings["PF_NotifyURL"]));
